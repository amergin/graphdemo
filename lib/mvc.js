...
jQuery( function($) 
{
	...
	window.GraphLegendView = Backbone.View.extend( {
		...
		initialize: function() {
			...
			window.App.Models.nodes.bind("reset", this.resetNumbers, this);
			App.Models.datasets.bind('change:active', this.resetNumbers, this);
			App.Models.nodes.bind('add', this.update, this);
		},
	});
	...
	window.OptionsMenuView = Backbone.View.extend({
		...
		_addItem: function() {
			this.renderEl.find("#items").append( this.itemTemplateCompiled( arguments[0].getRaw() ) );
		},

		_removeItem: function() {
			// remove div from list
			this.renderEl.find("[id=" + arguments[0].get("id") + "]").remove();
		},
