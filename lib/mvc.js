		...
		_show: function() {
			...
			this._showSelectInfo();
		      ...
		      {     	
		      	...
		      	{
		      		this.$.multiAccordion( { 'active': [ this.tabs['link'], this.tabs['plot'], this.tabs['boxplotinfo'] ] } );
					this.$('#plot').prev().children('a').fadeOut( function() { $(this).text(this.boxPlotText).fadeIn(); } );
		      		App.Sidebar.Plot.boxPlot( { categoricalO: this.model.getCategorical(), numericalO: this.model.getNumeric() } );
		      		App.Sidebar.Plot.boxPlotInfo({ categoricalO: this.model.getCategorical()});
		      	}
		      	else
		      	{
		      		this.$.multiAccordion( { 'active': [ this.tabs['link'], this.tabs['plot'] ] } );	      		
					this.$('#plot').prev().children('a').fadeOut( function() { $(this).text(this.boxPlotText).fadeIn(); } );
		      		App.Sidebar.Plot.boxPlot( { categoricalO: this.model.getCategorical(), numericalO: this.model.getNumeric() } );
		      	}
		      }
		        ...
		        ( sourceType ===  'C' && targetType === 'B' ) || ( sourceType ===  'C' && targetType === 'C' ) )
		      {
		      	...
		      	App.Sidebar.Plot.categorical({ sourceO: source, targetO: target });
		      }
		      else 
		      {
		      	// must be numerical
		      	this.$('#plot').prev().children('a').fadeOut( function() { $(this).text(this.scatterPlotText).fadeIn(); } );
		      	App.Sidebar.Plot.scatterplot({ sourceO: source, targetO: target });
		      }

		},

		_hide: function() {
			this.$.multiAccordion({ active: this.tabs.filter });
