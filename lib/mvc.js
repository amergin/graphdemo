		...
		_show: function() {
			...
			var self = this;
		      ...
		      {     	
		      	...
		      	if( App.Models.datasets.get('active').get('samples').length > 0 )
		      	{
					...
					this.$('#plot').prev().children('a').fadeOut( function() { $(this).text(self.boxPlotText).fadeIn(); } );
		      		_.delay( App.Sidebar.Plot.boxPlot, delay, { categoricalO: this.model.getCategorical(), numericalO: this.model.getNumeric() } );// ( { categoricalO: this.model.getCategorical(), numericalO: this.model.getNumeric() } );
		      		_.delay( App.Sidebar.Plot.boxPlotInfo, delay, { categoricalO: this.model.getCategorical()} );//({ categoricalO: this.model.getCategorical()});
		      	}
		      	else
		      	{
		        ...
		        ( sourceType ===  'C' && targetType === 'B' ) || ( sourceType ===  'C' && targetType === 'C' ) )
		      {
		      	...
		      	_.delay( App.Sidebar.Plot.categorical, delay, { sourceO: source, targetO: target }); // ({ sourceO: source, targetO: target });
		      }
		      else 
		      {
		      	// must be numerical
		      	...
		      	_.delay( App.Sidebar.Plot.scatterplot, delay, { sourceO: source, targetO: target } );//({ sourceO: source, targetO: target });
		      }
