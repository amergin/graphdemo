		...
		_show: function() {
			...
			this._showSelectInfo();
		      ...
		      {     	
		      	...
		      	if( App.Models.datasets.get('active').get('samples').length > 0 )
		      	{
					...
					this.$('#plot').prev().children('a').fadeOut( function() { $(this).text(this.boxPlotText).fadeIn(); } );
		      		_.delay( App.Sidebar.Plot.boxPlot, 300, { categoricalO: this.model.getCategorical(), numericalO: this.model.getNumeric() } );// ( { categoricalO: this.model.getCategorical(), numericalO: this.model.getNumeric() } );
		      		_.delay( App.Sidebar.Plot.boxPlotInfo, 300, { categoricalO: this.model.getCategorical()} );//({ categoricalO: this.model.getCategorical()});
		      	}
		      	else
		      	{
		        ...
		        ( sourceType ===  'C' && targetType === 'B' ) || ( sourceType ===  'C' && targetType === 'C' ) )
		      {
		      	...
		      	_.delay( App.Sidebar.Plot.categorical, 300, { sourceO: source, targetO: target }); // ({ sourceO: source, targetO: target });
		      }
		      else 
		      {
		      	// must be numerical
		      	this.$el.multiAccordion( { 'active': [ this.tabs['link'], this.tabs['plot'] ] } );
		      	this.$('#plot').prev().children('a').fadeOut( function() { $(this).text(this.scatterPlotText).fadeIn(); } );
		      	_.delay( App.Sidebar.Plot.scatterplot, 300, { sourceO: source, targetO: target } );//({ sourceO: source, targetO: target });
		      }
