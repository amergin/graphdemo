...
jQuery( function($) 
{
	...
	window.Dataset = Backbone.Model.extend( {
		...
		{
			console.log("creating Dataset ", this.name);
	...
	// 		_.bindAll(this, 'getNames', 'comparator', 'find', 'setActive');			

	// 		var self = this;
	// 	 	// initialize every dataset
	//       $.ajax( {
	//         url: this.defaults.fetchURL,
	//         type: this.defaults.fetchType,
	//         dataType: 'json',
	//         async: false,
	//         success: function(json) {
	//         	$.each( json.datalabels, function(ind, label)
	//         	{
	//         		self.models.push( new Dataset( { name: label } ) );
	//         	});
	//         }
	//     });
	//   },

	//   find: function(att, value) {
	//   	var search = {};
	//   	search[att] = value;
	//   	return this.where(search);
	//   },

	//   comparator: function(dset) {
	...
	// 	{
	// 		var names = [];
	// 		this.each( function(dset) { names.push( dset.get('name') ); } );
	// 		return names;
	// 	},

	// 	setActive: function( name )
	// 	{
	// 		this.active = this.find('name', name);
	// 	},
	...
	window.NodeCollection = Backbone.Collection.extend( {
		...
		getNodes: function() {
			var objects;
			this.models.each( function(ele) { objects.push( ele.getRaw() ) } );
			return objects;
		},
	...
	window.AppView = Backbone.View.extend({
		...
		initialize: function()
		{
	      ...
	      $('#filter').tabs( { selected: '#clinSearch' });	      
			...
			// header tab views
			var graphConf = { model: this.datasets, el: $('#graphTab.headertab'), tabId: 0 };
			graphConf.tabFunction = App.Graph.initGraph;

			var heatmapConf = { model: this.datasets, el: $('#heatmapTab.headertab'), tabId: 1 };
			graphConf.tabFunction = App.Graph.initHeatmap;

			var pathsConf = { model: this.datasets, el: $('#pathTab.headertab'), tabId: 2 };
			graphConf.tabFunction = App.Graph.initPaths;

			this.graph = window.App.Views.graph = new HeaderTabView( graphConf );
			this.heatmap = window.App.Views.heatmap = new HeaderTabView( heatmapConf );
			this.paths = window.App.Views.paths = new HeaderTabView( pathsConf );

			var views = {};
			views[App.Views.graph.el.id] = App.Views.graph;
			views[App.Views.heatmap.el.id] = App.Views.heatmap;
			views[App.Views.paths.el.id] = App.Views.paths;
	...
	window.HeaderView =  Backbone.View.extend({
		...
		initialize: function(views) {
			...
			$(this.el).bind('tabsselect', _.bind( function(event,ui) {
				// delegate the action to correct view
				this.views[ui.panel.id].render();
	...
	window.HeaderTabView = Backbone.View.extend({
		...
		initialize: function(options) {
			...
			_.bindAll(this);
			// notice the order: bind always the latter of the collections to be 
			// added; see updateElements
	...
	// extend custom functions to libraries
	window.App.Utilities.load();	
