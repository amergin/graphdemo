...
jQuery( function($) 
{
	...
	window.Dataset = Backbone.Model.extend( {
		...
		initialize: function(name)
		{
	        ...
	        $.ajax( {
	                type: this.defaults.fetchClinType,
	                url: this.defaults.fetchClinURL,
	                dataType: "json",
	                //async: false,
	                data: JSON.stringify( { datalabel: self.name } ),
	                contentType: "application/json",
	                success: function( json )
	                {
	                	self.clinicalNodes = json.nodes;
	                },
	        ...
	        $.ajax( {
	                type: this.defaults.fetchSampleType,
	                url: this.defaults.fetchSampleURL,
	                dataType: "json",
	                //async: false,
	                data: JSON.stringify( { datalabel: self.name } ),
	                contentType: "application/json",
		...
		initialize: function() 
		{
	      ...
	      $.ajax( {
	        ...
	        success: function(json) {
	        	...
	        	{
	        		self.datasets.push( new Dataset( label ) );
	        	});
	        }
	    });
	  },
	...
	window.Node = Backbone.Model.extend( {
		...
		{
		      $.ajax( {
		        url: this.defaults.fetchPatientsURL,
		        type: this.defaults.fetchType,
		        dataType: 'json',
		        data: JSON.stringify( { nodeId: this.id } ),
		        async: false,
		        success: function(json) {
		        	this.patientvals = json.patients.split(this.defaults.patientsDelim);
		        }
		    });
		}
	});
