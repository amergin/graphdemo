...
jQuery( function($) 
{
	...
	window.ApplicationRouter = Backbone.Router.extend({
		...
		neighborhood0: function( headerTab, filterTab, dataset, nodeType, nodeIdentification, depth, nodeNo, edgeAttr, edgeOrdering )
		{
			...
			$('#' + filterTab + ' #depth').val(depth);
			$('#' + filterTab + ' select#nodes').val(nodeNo);
			...
			this.navigate( ["neighborhood", headerTab, filterTab, dataset, nodeType, nodeIdentification, depth, nodeNo, edgeAttr, edgeOrdering ].join("/") );
		},

		neighborhood1: function( headerTab, filterTab, dataset, nodeType, nodeIdentification, depth, nodeNo, edgeAttr, edgeOrdering, firstEdge )
		{
			var utils = App.Utilities;
			if( !utils.legalDataset(dataset) || !utils.legalNodeType(nodeType) || 
			...
			// change the active tab
			$('#headertabs').tabs('select', '#' + headerTab);
			this.navigate( ["neighborhood", headerTab, filterTab, dataset, nodeType, nodeIdentification, depth, nodeNo, edgeAttr, edgeOrdering, firstEdge ].join("/") );
		},

		neighborhood2: function( headerTab, filterTab, dataset, nodeType, nodeIdentification, depth, nodeNo, edgeAttr, edgeOrdering, firstEdge, secondEdge )
		{
			var utils = App.Utilities;
			if( !utils.legalDataset(dataset) || !utils.legalNodeType(nodeType) || 
			...
			// change the active tab
			$('#headertabs').tabs('select', '#' + headerTab);
			this.navigate( ["neighborhood", headerTab, filterTab, dataset, nodeType, nodeIdentification, depth, nodeNo, edgeAttr, edgeOrdering, firstEdge, secondEdge ].join("/") );
		},
		...
		{
			var utils = App.Utilities;
			var filterTab = 'regulatory';
			if( !utils.legalTab( headerTab ) || !utils.legalDataset(dataset) || 
				!utils.legalNodeType(middle) || !utils.legalNodeType(target) ||
				( mutatedType !== "" && !utils.legalMutated(mutatedType) ) )
			{
	...
	window.AppView = Backbone.View.extend({
		...
		initialize: function()
		{
	      ...
	      $('#filter').tabs( { selected: '#clinSearch' });	      
			...
			// no functionality:
			//this.heatmap.selectLink = undefined;
			...
			this.pahts.selectLink = App.Paths.selectLink;
