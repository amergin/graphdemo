...
jQuery( function($) 
{
	...
	window.AppView = Backbone.View.extend({
		...
		initialize: function()
		{
	      ...
	      $('#filter').tabs( { selected: '#clinSearch' });	      
			...
			this.graph = window.App.Views.graph = new HeaderTabView( { 
				model: this.datasets, 
				model2: App.Models.graph, 
				el: $('#graphTab.headertab'), tabId: 0, 
				bindFunction: function() { 
					var self = this;
					this.$('#export').live("click", function(event) { 
						...
						var form = $('<form/>', {
							...
							type: "hidden",
							name: 'filename',
							value: App.Models.datasets.get('active').get('name') + "-" + App.Models.nodes.find('id', 'n' + App.Models.datasets.get('active').get('referenceNodeId'))[0].get('label')
						}) ).append( $("<input>", {
							type: "hidden",
							name: 'filetype',
							value: 'svg'
							...
							type: "hidden",
							name: 'contents',
							value: (new XMLSerializer).serializeToString( document.getElementById('forcegraph') ) //App.Utilities.Base64.encode( (new XMLSerializer).serializeToString( document.getElementById('forcegraph') ) ) // App.Utilities.Base64.encode( (new XMLSerializer).serializeToString( document.getElementById('forcegraph') ) ) //App.Utilities.Base64.encode( document.getElementById('forcegraph').innerSVG )
						}) );
						$('body').append(form);
						form.submit();
						form.remove();
				...
				tabFunction: App.Graph.initGraph,
				hideLink: App.Graph.hideLink,
				selectLink: App.Graph.selectLink
			} );
			...
			this.graphLegend = window.App.Views.graphLegend = new GraphLegendView();
			this.heatmapLegend = window.App.Views.heatmapLegend = new HeatmapLegendView();
	...
	window.HeatmapLegendView = Backbone.View.extend( {
		el:  $('#heatmap-legend-sidePanel'),
		catTmpl : $('#heatmap-legend-categorical-tmpl'),
		forcedVisible: false,

		events: {
			"click input[type=checkbox]": "keepVisible"
		},

		keepVisible: function() {
			this.forcedVisible = this.$('input#keepVisible').is(":checked");
		},
		...
		init: function() {
	        ...
	        this.$('#heatmap-legend-panelHandle').hover(function() {
	            $('#heatmap-legend-sidePanel').stop(true, false).animate({
	                'left': '0px'
	            }, 500);
	        }, function() {
	            // jQuery.noConflict();
	        ...
	        var self = this;
	        jQuery('#heatmap-legend-sidePanel').hover(function() {
	            // Do nothing
	        }, function() {
	        	if( self.forcedVisible ) return;
	            setTimeout( function() { jQuery('#heatmap-legend-sidePanel').animate({
	                left: '-351px'
	            }, 800); }, 1000 );
	        });

	        this.catTmplCompiled = Handlebars.compile( this.catTmpl.html() );
		},

		render: function(options) {
			this.empty();
			this.$('#heatmap-legend-panelContent').append( $('<input>', 
				{ type: "checkbox", id: "keepVisible", title: "Keep side panel visible", checked: this.forcedVisible }) );
			...
			var colorRange,
				...
				thElement,
				sampleType = 'N',
				numericalTable = $('<table/>', { id: "numerical"})
					.append("<thead><tr></tr></thead>")
					.append("<tbody><tr></tr></tbody>")
					.appendTo("#heatmap-legend-panelContent");

			var getHeaderHTML = function(nodeSource) {
				var titles = {
					CLIN: 'Clinical',
				...
				return $('<img/>', {
					class: "nodeTableImg",
					src: "img/node_" + nodeSource.toLowerCase() + ".svg",
					title: titles[nodeSource]
				})[0].outerHTML + nodeSource;
			}

			// draw all numeric bars:
			$.each( options.values[sampleType], function(id, valRange) {
					if( valRange.length === 0 ) return true;
					if( !options.colors[sampleType].hasOwnProperty(id) ) {
						colorRange = _.values( _.omit( options.colors[sampleType]['other'], 'range' ) );
						valueRange = options.colors[sampleType]['other'].hasOwnProperty('range') ?  options.colors[sampleType]['other']['range'] : valRange;
					}
					else {
						colorRange = _.values( _.omit( options.colors[sampleType][id], 'range' ) );
						valueRange = options.colors[sampleType][id].hasOwnProperty('range') ?  options.colors[sampleType][id]['range'] : valRange;
					}

					thElement = $('<th/>', {}).append(getHeaderHTML(id));
					numericalTable.find("thead tr").append(thElement);
					tdElement = document.createElement("td");
					var svg = d3.select(tdElement)
						.append("svg")
					    .attr("width", 65)
					    .attr("height", 200),
					g = svg.append("g").attr("transform","translate(10,10)").classed("colorbar",true),
					cb = App.Heatmap.colorBar()
						.color(d3.scale.linear()
						.domain(valueRange).range( colorRange ) ).size(180).lineWidth(20).precision(2);
					g.call(cb);
					numericalTable.find('tbody tr').append(tdElement);
			});
			numericalTable.parent().prepend("<h3>Numerical values</h3>").append("<hr>");

			// add categories table
			var noCategoricals = _.size( options.values['C'] );
			if(!noCategoricals) return;

			var obj = {
				cols: _.max( _.map( options.values['C'], function(ele,ind) { return _.values(ele).length } ) ) + 1, // 1 comes from left-side th's
				nodes: _.map( options.values['C'], function(val,prop) { return { 
					origLabel: App.Models.nodes.find('id', prop)[0].get('label'),
					label: App.Utilities.truncateLabel( App.Models.nodes.find('id', prop)[0].get('label') ),
					cats: _.map(val, function(ele,ind) { return { color: options.colorScales['C'][prop][ele], name: ele } } )
				}; } )
			};
			this.$('#heatmap-legend-panelContent').append( this.catTmplCompiled( obj ) );
			this.$('#heatmap-legend-panelContent').find('#categorical tbody td, #categorical tbody th').qtip({ 
		      ...
		      style: {
		        classes: "ui-tooltip-dark ui-tooltip-shadow tooltip"
		      }	          
			});
		},

		empty: function() {
			this.$('#heatmap-legend-panelContent').empty();
		},

		toggle: function() {
			if( App.Views.header.getActive().attr('id') === "heatmapTab" ) this.show();
			else this.hide();			
		},
		...
		initialize: function() {
			...
			App.Utilities.eventAggregator.bind("change:tab", this.toggle, this);
			this.init();

			window.App.Models.nodes.bind("reset", this.empty, this);
	...
	window.GraphLegendView = Backbone.View.extend( {
		...
		init: function() {
	        ...
	        jQuery('#forcegraph-legend-sidePanel').hover(function() {
	        	...
	        	if( self.forcedVisible ) return;
	        	// hide afterwards, delay
	            setTimeout( function() { jQuery('#forcegraph-legend-sidePanel').animate({
	                left: '-251px'
	            }, 800); }, 1000 );
	        });
		...
		toggle: function() {
			if( App.Views.header.getActive().attr('id') === "graphTab" ) this.show();
			else this.hide();
		},
		...
		initialize: function() {
			...
			window.App.Models.nodes.bind("reset", this.update, this);
		},
	});
	...
	window.OptionsMenuView = Backbone.View.extend({
		...
		// graphs
		cssGraphHighlight: "fill: #68CC68; font-size: 17px; stroke: black; stroke-width: 1px; font-weight: bold; letter-spacing: 0.20em;",
		// path view
		...
		_addItem: function() {
			...
			this.renderEl.find("#items").prepend( this.itemTemplateCompiled( node ) );
		...
		_inform: function() {
			...
			var overlay = $('#options-overlay h1').text(text).parent().show();
			setTimeout( function() { overlay.fadeOut('slow', function() { overlay.hide().css("opacity", "").find("h1").text(""); } ) }, 2000 );
		},
		...
		initialize: function() {
			...
			// reset item list when dataset changes
			App.Models.datasets.bind('change:active', this._reset, this);
		...
		_applyAutocomplete: function() {
			...
			searchdivEle.find('input#labelSearch').on("autocompleteselect", function(event, ui) {
				...
				$(this).val("");
				event.preventDefault(); // prevents populating the field
			});
		},
	...
	window.HeaderView =  Backbone.View.extend({
		...
		initialize: function(options) {
			...
			this.$el.bind('tabsselect', function(event, ui) {
				setTimeout( function() { App.Utilities.eventAggregator.trigger("change:tab") }, 100 );
			});
			...
			this.$el.bind('tabsshow', _.bind( function(event,ui) {
				...
				// fire custom event
				App.Utilities.eventAggregator.trigger("change:tab");
			}, this) );			
		},
	...
	window.HeaderTabView = Backbone.View.extend({
		...
		initialize: function(options) {
			...
			if( this.hasOwnProperty('bindFunction') ) this.bindFunction();
		},

		render: function() { 
			// render only if shown
			if( $('#headertabs').tabs('option','selected') === this.tabId ) {
				if( App.Models.nodes.length > 0 ) this.showTab();
			}
		},
