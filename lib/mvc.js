	...
	window.HeatmapLegendView = Backbone.View.extend( {
		...
		render: function(options) {
			...
			$.each( options.values[sampleType], function(id, valRange) {
					...
					// // sometimes all values are coincidentally the same
					// if( valueRange.length === 2 && valueRange[0] === valueRange[1] )
					// {
					...
					console.log("ongelmakohta", id);
					var svg = d3.select(tdElement)
						.append("svg")
					    .attr("width", 65)
					    .attr("height", 200),
					g = svg.append("g").attr("transform","translate(10,10)").classed("colorbar",true),
					cb = App.Heatmap.colorBar().color(d3.scale.linear().domain(valueRange).range( colorRange ) ).size(180).lineWidth(20).precision(3);
					g.call(cb);
