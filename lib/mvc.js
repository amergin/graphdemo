	...
	window.HeatmapLegendView = Backbone.View.extend( {
		...
		init: function() {
	        ...
	        jQuery('#heatmap-legend-sidePanel').hover(function() {
	            ...
	            // jQuery.noConflict();
	            jQuery('#heatmap-legend-sidePanel').animate({
	                left: '-351px'
	            }, 800);
	        });
		},

		addColorBars: function(options) {
			...
			$.each( options.values[sampleType], function(id, valRange) {
					...
					// console.log(id, valRange);
					// console.log("sampleType" + sampleType);
					// console.log( valRange, colorRange );


					// console.log(id, valueRange, colorRange );
					thElement = $('<th/>', {}).text(id);
					numericalTable.find("thead tr").append(thElement);
					tdElement = document.createElement("td");
					var svg = d3.select(tdElement)//d3.select("#heatmap-legend-panelContent table#numerical tbody tr")
						.append("svg")
					    .attr("width", 60)
					    .attr("height", 220),
			...
			// this.$el('heatmap-legend-panelContent')
		},
