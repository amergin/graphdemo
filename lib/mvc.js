...
jQuery( function($) 
{
	...
	window.SidebarView = Backbone.View.extend({
		...
		render: function() {
			...
			if( !this.model.get('nodeA') ) { this._hide(); return; }

			this._show();
		},

		_show: function() {
			var sourceType = this.model.get('nodeA').get('type');
			var targetType = this.model.get('nodeB').get('type');
			App.Sidebar.Info.selectInfo();
		      ...
		      if( ( sourceType ===  'B' && targetType === 'B' ) || ( sourceType ===  'B' && targetType === 'C' ) ||
		...
		_hide: function() {
			this.$el.multiAccordion({
				active: 0,
				click: function(event,ui) { console.log(event,ui); }
			});
			$('div#boxplotinfo.sidetab').prev().css('display','');
