...
jQuery( function($) 
{
	...
	window.ApplicationRouter = Backbone.Router.extend({
		routes: {
			...
			"neighbor/:filterTab/:dataset/:nodeType/:nodeIdentification/:depth/:nodeNo/:edgeAttr/:edgeOrdering/:headerTab" : 'neighborhood0',
			"neighbor/:filterTab/:dataset/:nodeType/:nodeIdentification/:depth/:nodeNo/:edgeAttr/:edgeOrdering/:headerTab/:firstEdge" : 'neighborhood1',
			"neighbor/:filterTab/:dataset/:nodeType/:nodeIdentification/:depth/:nodeNo/:edgeAttr/:edgeOrdering/:headerTab/:firstEdge/:secondEdge" : 'neighborhood2',
		...
		neighborhood0: function( filterTab, dataset, nodeType, nodeIdentification, depth, nodeNo, edgeAttr, edgeOrdering, headerTab )
		{
			...
			this.navigate( "neighbor" + "/" + filterTab + "/" + dataset + "/" + nodeType + "/" + nodeId + "/" + depth + "/" + nodeNo + "/" + edgeAttr + "/" + edgeOrdering + "/" + headerTab );
		},

		neighborhood1: function( filterTab, dataset, nodeType, nodeIdentification, depth, nodeNo, edgeAttr, edgeOrdering, headerTab, firstEdge )
		{
			var utils = App.Utilities;
			if( !utils.legalDataset(dataset) || !utils.legalNodeType(nodeType) || 
				!utils.legalNodeType(firstEdge) || !utils.legalNodeType(secondEdge) ||
			...
			this.navigate( "neighbor" + "/" + filterTab + "/" + dataset + "/" + nodeType + "/" + nodeId + "/" + depth + "/" + nodeNo + "/" + edgeAttr + "/" + edgeOrdering + "/" + headerTab + "/" + firstEdge );
		},

		neighborhood2: function( filterTab, dataset, nodeType, nodeIdentification, depth, nodeNo, edgeAttr, edgeOrdering, headerTab, firstEdge, secondEdge )
		{
			var utils = App.Utilities;
			if( !utils.legalDataset(dataset) || !utils.legalNodeType(nodeType) || 
				!utils.legalNodeType(firstEdge) || !utils.legalNodeType(secondEdge) ||
			...
			this.navigate( "neighbor" + "/" + filterTab + "/" + dataset + "/" + nodeType + "/" + nodeId + "/" + depth + "/" + nodeNo + "/" + edgeAttr + "/" + edgeOrdering + "/" + headerTab + "/" + firstEdge + "/" + secondEdge );
		},


		//:dataset/:nodeIdentification/:middle/:target/:headerTab/*mutatedType
		regulatory: function( filterTab, dataset, nodeIdentification, middle, target, headerTab, mutatedType )
		{
			if( !utils.legalDataset(dataset) || !utils.legalNodeType(nodeType) || 
			...
			if( mutatedType )
			{
				this.navigate("regulatory/" + dataset + "/" + nodeIdentification + "/" + middle + "/" + target + "/" + headerTab + "/" + mutatedType);
			}
			else {
				this.navigate("regulatory/" + dataset + "/" + nodeIdentification + "/" + middle + "/" + target + "/" + headerTab);			
			}
		},
	...
	window.ClinicalSearchView = Backbone.View.extend({
		...
		{	
			...
			event.preventDefault();
              ...
              var route = "neighbor";
              route += "/" + this.$el.attr('id');
              route += "/" + App.Models.datasets.get('active').get('name');
              route += "/" + neighborhoodQuery.nodeType;
              route += "/" + neighborhoodQuery.nodeId;
              route += "/" + neighborhoodQuery.depth;
              route += "/" + neighborhoodQuery.nodes;
              route += "/" + neighborhoodQuery.edgeOrderingAttribute;
              route += "/" + neighborhoodQuery.edgeOrdering;
              route += "/" + App.Views.header.getActive().attr('id');

              if( this.$el('select[id="firstEdgeType"]').val() !== 'Any' )
              {
                neighborhoodQuery['firstEdgeType'] = this.$('select[id="firstEdgeType"]').val();
                route += "/" + neighborhoodQuery.firstEdgeType;
              }

              if( this.$el('select[id="secondEdgeType"]').val() !== 'Any' )
              {
                neighborhoodQuery['secondEdgeType'] = this.$('select[id="secondEdgeType"]').val();
                route += "/" + neighborhoodQuery.secondEdgeType;               
              }

              App.Routers.app.navigate( route );
	...
	window.RegulatorySearchView = Backbone.View.extend({
		...
		submitSearch: function(event) {
          ...
          //"regulatory/:dataset/:nodeIdentification/:middle/:target/:headerTab/*mutatedType"
          var route = this.$el.attr('id');
          route += "/" + App.Models.datasets.get('active').get('name');
          route += "/" + query.clinicalNodeId;
          route += "/" + query.middleNodeType;
          route += "/" + query.targetType;
          route += "/" + App.Views.header.getActive().attr('id');
          route += "/" + query.mutatedType;
          App.Routers.app.navigate( route );
