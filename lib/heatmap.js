...
window.App.Heatmap.initHeatmap = function()
{
  ...
  var refNode = App.Models.nodes.find('id', refNodeId);
  ...
  $.each( edges, function(ind, edge)
  {
    ...
    {
      neighbors.push( App.Models.nodes.find('id', edge.target) );
    }
  });
...
window.App.Heatmap.renderHeatmap = function( patients, nodes, refNode, config )
  {
    ...
    {
      var refNodeNonNAPatients = $.grep( refNode.patientvals, function(ele) { return ele !== "NA" } ).length;
    }
    ...
    {
      nodeLabel = node.label || createNodeLabel( node );
      if( node.type === 'N' )
      {
        $("<th/>",
        {
        ...
        .appendTo( "#numerical_container table thead tr" ).html( '<div class="nodeLabelHeader">' + truncateLabel( nodeLabel ) + '</div>' );
      ...
      else if( node.type === 'C' || node.type === 'B' )
      {
        ...
        .appendTo( "#categorical_container table thead tr" ).html( '<div class="nodeLabelHeader">' + truncateLabel( nodeLabel ) + '</div>' );
