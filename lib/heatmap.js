      ...
      $.each( nodeset, function(source, sourceNodes)
      {
        ...
        else // categorical
        {
          $.each( sourceNodes, function(ind,node)
          {
            ...
            minMaxCatalog[type][node.id] = distinctVals; // for legend
...
if( sampleClassPresent )
{
  ...
  $.each( sampleDistinctVals, function(ind,val)
  {
    ...
    App.Views.heatmapLegend.render({ values: minMaxCatalog, colors: config.colors, colorScales: colorRangeCatalog });
        ...
        $.each( indices, function(arInd, indValue)
        {
              ...
              $.each( nodes, function(nodeI, node)
              {
                if( node.type === 'N' )
                {
                  $("<td/>", {})
                  .css({
                    background: getColor( node.patientvals[indValue], node ),
                    width: numericalCellWidth + "px",
                    height: numericalCellHeight + "px"
                  })
                ...
                else if( node.type === 'C' || node.type === 'B' )
                {
                  ...
                  .css({
                    background: getColor( node.patientvals[indValue], node ),
                    width: categoricalCellWidth + "px",
                    height: categoricalCellHeight + "px"
                  })                  
  ...
  $.each( patients, function(indY, patient)
  {
        ...
        $.each( nodes, function(indX, node)
        {
          if( node.type === 'N' )
          {
            $("<td/>", {})
            .css({
              background: getColor( node.patientvals[indY], node ),
              width: numericalCellWidth + "px",
              height: numericalCellHeight + "px"
            })
            .appendTo( numTable.find("tbody tr:last") );
    ...
    if( sampleClassPresent )
    {
        ...
        $.each( patients, function(indY, patient)
        {
          ...
          .css({
            width: sampleContainerWidth,
            height: numericalCellHeight,
            background: getSampleColor( patient.class )
          })
        ...
        sampleTable.find("tbody td").qtip({
            content: function(api) {
              return "<b>Patient</b>: " + $(this).attr("name") + "<br><b>Class</b>: "  + $(this).attr("value");
            },
            position: { 
              viewport: $(window),
              my: 'left center',
