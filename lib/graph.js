...
// called when a link is selected in any tab view (sidebar model binding)
/*window.App.Graph.selectLink = function(d)
{
  var conf = App.Graph.config;
    ...
    if( selectedLink )
    {
      // selected link is clicked to unselect this
      if( selectedLink.id == this.id )
      {
        d3.select( this ).style("stroke", conf.linkDefaultColor);
        App.Models.sidebar.reset();
        return;
      }
      // other link is clicked to change selection
      d3.select('line#' + selectedLink.id + '.link').style("stroke", conf.linkDefaultColor);
      App.Models.sidebar.reset();
    }
    d3.select("line.link#" + d.id).style("stroke", conf.linkSelectedColor);
      ...
      for (var i = nodeObjects.length * nodeObjects.length * 3 + 1000; i > 0; --i) force.tick();
        ...
        .attr("id", function(d) { return d.id; } )
            ...
            .on("mouseover", function(d) {
              ...
              selection.transition().duration(50).style("stroke-width", initialWidth + Number(4) )
              .style("stroke-opacity", 1.0);
              //.style("stroke", linkOverColor);
            } )
            .on("mouseout", function(d) {
              var selection = d3.select(this);
              selection.transition().duration(50).style("stroke-width", getLinkStroke( selection.data()[0] ) )
...
.on("click", function(d) { 
  ...
  console.log("nodeclick");
        ...
        // return to default view
        .on("mouseout", function(d) {
          // don't modify subgraph clicks
          //if( App.Models.graph.get('clickedNodes').length > 0 ) return;
          // only when no selected nodes are present
          fade(conf.node_def_opacity) 
        });
