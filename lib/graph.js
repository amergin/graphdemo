...
window.App.Graph.render = function()
{
  ...
  var getLinkStroke = function( edgeO )
  {
    ...
    return Number(edgeO.pvalue) * 0.4 + 3 || Number(4);
      ...
      .attr("refX", 25)
      // !IMPORTANT: although these are part of the standard, Inkscape will not use them and this
      // will screw up your graph in it, use with caution
      ...
      for (var i = nodeObjects.length * nodeObjects.length * 3 + 1000; i > 0; --i) force.tick();
        ...
        force.on("tick", function()
        {
          ...
          // move nodeText to new position
          nodeText
          .attr("x", function(d) { return d.x } )
          .attr("y", function(d) { return d.y - (conf.nodeImageH * d._scale / 2) - 5; } );
        });
