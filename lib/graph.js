      ...
      for (var i = nodeObjects.length * nodeObjects.length * 3 + 1000; i > 0; --i) force.tick();
        ...
        .attr("id", function(d) { return d.id; } )
            ...
            .on("mousedown", function(d) {
              ...
              if( App.Models.sidebar.get('link') )
              {
                // selected link is clicked to unselect this
                if( App.Models.sidebar.get('link').id == this.id )
                {
                ...
                d3.select('line#' + App.Models.sidebar.get('link').id + '.link').style("stroke", conf.linkDefaultColor);
                App.Models.sidebar.reset();
              }
