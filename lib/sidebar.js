App.Sidebar.styleForm = function( searchdivEle ) {

      // formatting function for dropdowns
      var addressFormatting = function(text){
        ...
        // style each dropdown
        searchdivEle.find(".dropdown").each( function(ind,ele) {
          var ele = $(this);
          var width = ele.attr('overviewWidth');
          var menuWidth = ele.attr('menuWidth');
          ele.selectmenu({
            style: "dropdown",
            width: width, 
            menuWidth: menuWidth,
            ...
            change: function() {
              ...
              if( arguments[0] && arguments[0].target.id === 'nodeType' ) searchdivEle.find('#labelSearch').val("");
            }
          });
        });

        if( searchdivEle.attr('id') === 'freeSearch')
        {
            // add autocomplete
        ...
        // style search button
        $(searchdivEle).find("input[type='submit']").button();
...
window.App.Sidebar.Plot.categorical = function(params)
{
  ...
  chartConfig.exporting.filename = App.Models.datasets.get('active').get('name') + "_" + ( sourceLabel || "(none)" ) + "_and_" + ( targetLabel || "(none)" );
  ...
  var getCategories = function( array1, array2, label1, label2 )
  {
    ...
    var catName;
      ...
      {
        if( categories.categories[cat] > 0 ) chartConfig.series[0].data.push( { y: categories.categories[cat], originalLabel: categories.originalNames[cat] } );
      });
    ...
    window.App.Sidebar.Plot.BoxPlotInfoConfig = {
      exporting: {
        ...
        window.App.Sidebar.Plot.boxPlotInfo = function(params)
        {
          ...
          // dont use old values
          chartConfig.series = [];
          chartConfig.exporting.filename = App.Models.datasets.get('active').get('name') + "_" + ( params.categoricalO.label || "(none)" );
          chartConfig.title['text'] = "Patient values of " + "<b>" + App.Utilities.truncateLabel( params.categoricalO.label ) + "</b>";
...
window.App.Sidebar.Plot.BoxPlotConfig = {
 ...
 exporting: {
  ...
  title: {
    ...
    tooltip:{
     formatter: function()
     {
       ...
       // add samplecount using external object
       s += ['<br>', '<span style="color:black">', 'Samples', '</span>: ',
       '<b>',  Highcharts.numberFormat( App.Sidebar.Plot.boxPlotCategories[this.x].length, 0 ), '</b>'].join('');
       ...
       for(var i = this.points.length-1; i>-1; i--)
       {
         ...
         '<b>', Highcharts.numberFormat(this.points[i].y, 2), '</b>'].join('');
       }
       return s;    
     },
     shared:true,
   ...
   xAxis:{
    ...
    labels: {
      ...
      formatter: function() { return App.Utilities.truncateLabel( this.value ); }
    }      
  },
  yAxis:{
    labels:{
  ...
  conf.xAxis.labels['x'] = -conf.custom.boxWidth;
  conf.xAxis.categories = [];
  conf['series'] = [];
  conf.yAxis.title['text'] = App.Utilities.createNodeLabel2( params.numericalO );
  conf.exporting.filename = App.Models.datasets.get('active').get('name') + "_Cat_" + ( params.categoricalO.label || "(none)" ) + "_and_Num_" + ( params.numericalO.label || "(none)");
  ...
  $.each( seriesNames, function(ind,name) {
    ...
    // add category names
    conf.xAxis.categories = _.keys( categories );
...
// share categories for later tooltip use
window.App.Sidebar.Plot.boxPlotCategories = categories;
...
window.App.Sidebar.Plot.scatterplot = function(params)// sourceNodeo, targetNodeo )
{
  ...
  // don't use previous values
  var scatterplotData = [];
  var sampleSeries = {};
  var dataX, dataY;
  var colorpalette = d3.scale.category10();

  scatterplotConfig.xAxis.title['text'] = App.Utilities.createNodeLabel2( params.sourceO ); //params.sourceO.source + " of " + params.sourceO.label + ", chr " + params.sourceO.chr;
  scatterplotConfig.yAxis.title['text'] = App.Utilities.createNodeLabel2( params.targetO ); //params.targetO.source + " of " + params.targetO.label + ", chr " + params.targetO.chr;
  scatterplotConfig.series = [];
  scatterplotConfig.exporting.filename = App.Models.datasets.get('active').get('name') + "_" + params.sourceO.label + "_and_" + params.targetO.label;  
