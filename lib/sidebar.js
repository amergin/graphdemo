...
window.App.Sidebar.Plot.boxPlot = function(params)
{
  ...
  $.each( seriesNames, function(ind,name) {
    ...
    $.each( categories, function(category, values) {
      ...
      series.data.push( _.min( values ) );
      series.data.push( d3.quantile( values, 0.25 ) );
      series.data.push( d3.median( values ) );
      series.data.push( d3.quantile( values, 0.75 ) );
      series.data.push( _.max( values ) );

      conf.plotOptions.series.push( series );
    });
  });
  ...
  var drawingFunction = function(chart) {
    ...
    for(i=0; i<quartile1.length; i++)
    {
        ...
        chart.renderer.rect(
             quartile3[i].plotX-(rectWidth/2)+chart.plotLeft-translate, //x
       ...
       // max value line
       chart.renderer.path(['M', max[i].plotX-(rectWidth/2)+chart.plotLeft-translate, max[i].plotY+chart.plotTop,
        'L', max[i].plotX+(rectWidth/2)+chart.plotLeft-translate, max[i].plotY+chart.plotTop ])
       .attr({
        'stroke-width': 1,
        stroke: 'blue',
        zIndex:5
        ...
        // median value line
        chart.renderer.path(['M',median[i].plotX-(rectWidth/2)+chart.plotLeft-translate,median[i].plotY+chart.plotTop,
          'L',median[i].plotX+(rectWidth/2)+chart.plotLeft-translate,median[i].plotY+chart.plotTop])
        .attr({
          'stroke-width': 1,
          stroke: 'green',
          zIndex:5
        ...
        // minimum value line
        chart.renderer.path(['M', min[i].plotX-(rectWidth/2)+chart.plotLeft-translate, min[i].plotY+chart.plotTop,
          'L', min[i].plotX+(rectWidth/2)+chart.plotLeft-translate, min[i].plotY+chart.plotTop])
        .attr({
          'stroke-width': 1,
          stroke: 'red',
          zIndex:5
