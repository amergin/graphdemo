...
window.App.Sidebar.Plot.boxPlot = function(params)
{
  ...
  var categories = {};
  var seriesNames = ['Minimum', '1st Quartile', 'Median', '3rd Quartile', 'Maximum', 'Samples'];
  var dataIndices = { 'min': 0, 'quartile1': 1, 'median': 2, 'quartile3': 3, 'max': 4, 'count': 5 };
  ...
  $.each( seriesNames, function(ind,name) {
    ...
    $.each( categories, function(category, values) {
      ...
      switch( ind )
      {
        ...
        case dataIndices['count']: series.data.push( values.length ); break;
      }
    });
