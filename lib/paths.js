...
window.App.Paths.initPaths = function()
{
  ...
  {
    return conf.nodeTemplate($.extend( nodeO, { imageSource: "img/node_" + nodeO.source.toLowerCase() + ".svg" } ) );
  };
  ...
  dfs( refNode );
    ...
    var tableConfig = {
        ...
        "fnInitComplete": function() {
          ...
          $(App.Paths.config.container + ' td.pathLink').on("click", function() {
            var link = App.Models.edges.find("id", $(this).children("div").attr("data-linkId"))[0].getRaw();
            link = $.extend(true, {}, link); // deep copy
            link.source = App.Models.nodes.find("id", link.source)[0].getRaw();
            link.target = App.Models.nodes.find("id", link.target)[0].getRaw();
    ...
    if( selectedLink )
    {
      App.Paths.selectLink({current: selectedLink});
    }
};