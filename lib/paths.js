...
window.App.Paths.initPaths = function()
{
  ...
  var edges = App.Models.edges.getEdges();
  var refNodeId = "n" + App.Models.datasets.get('active').get('referenceNodeId');
  var refNode = App.Models.nodes.find('id', refNodeId)[0];
  ...
  {
    visitedCatalog[nodeO.get('id')] = 'gray';
    ...
    traversingStack.pop();
    visitedCatalog[nodeOd.get('source')] = 'black';
  };

  // mark down links that leave from each node
  $.each( edges, function(i,d)
  {
    linkSourceDestCatalog[ d.get('source') + "|" + d.get('target') ] = d;
    // if( !nodeLinkCatalog.hasOwnProperty( d.target.id ) )
    // {
    //   nodeLinkCatalog[ d.target.id ] = [];
    // }
    // nodeLinkCatalog[ d.target.id ].push( d );
    if( !nodeLinkCatalog.hasOwnProperty( d.get('source') ) )
    {
      nodeLinkCatalog[ d.get('source') ] = [];
    }
    nodeLinkCatalog[ d.get('source') ].push( d );
  });
    ...
    var tableConfig = {
        ...
        $.each( simplePaths, function(indP, pathElements)
        {
          ...
          $.each( pathElements, function(indN,nodeO)
          {
            ...
            {
              pathCell['link' + (indN+1)] = getLinkHTML( linkSourceDestCatalog[ nodeO.get('id') + "|" + pathElements[indN+1].get('id') ] );
            }
          ...
          {
            pathLinks.push( linkSourceDestCatalog[ pathElements[i-1].get('id') + "|" + pathElements[i].get('id') ] );
          }
