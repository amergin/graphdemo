  ...
  dfs( refNode );
    ...
    var tableConfig = {
        ...
        $.each( simplePaths, function(indP, pathElements)
        {
          ...
          {
            // notice the indexing: later used indN+1!
            if( indN >= ( pathElements.length - 1 ) )
            {
            ...
            {
              nodeObject = pathElements[indN];
              pathCell['node' + (indN+1)] = getNodeHTML( nodeObject );
              pathCell['link' + (indN+1)] = ( linkSourceDestCatalog[ nodeObject.id + "|" + pathElements[indN+1].id ] );
            }
          }
